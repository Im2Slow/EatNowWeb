...
<label for="email">Email : </label><input style="width: 250px;"type="text" name="email" maxlength="50" value="<?php if (!empty($_POST["email"])) { echo stripcslashes(htmlspecialchars($_POST["email"],ENT_QUOTES)); } ?>" /><br/>
...
if(isset($_POST['Valider'])){
    ...
    //c'est ok
        ...
        {
            echo '<div id="erreur">Aucunes donn&eacute;es ne correspond &agrave; votre saisie!</div>';
        }
        //si c'est ok
        else{
        //on boucle pour récupérer le pseudo et pass du membre pour lui envoyer
            while($result = mysql_fetch_array($index)){
                ...
                echo '<div id="ok">Vos identifiants ont &eacute;t&eacute; envoy&eacute;s sur votre bo&icirc;te email.</div>                         <script type="text/javascript"> window.setTimeout("location=(\'index.php?recup=ok\');",3000) </script>';                
            }
        }
        close_bd();    
    }
...
echo '<div id="ok">Inscription r&eacute;ussie. Un message vous a &eacute;t&eacute; envoy&eacute; sur votre bo&icirc;te email. Merci de cliquer sur le lien pr&eacute;sent dans celui-ci pour valider votre inscription.</div>';
}
if(isset($_GET['valide']) && $_GET['valide']=="ok"){
echo '<div id="ok">Inscription valid&eacute;e avec succ&egrave;s! Vous pouvez vous identifier.</div>';
}
if(isset($_GET['recup']) && $_GET['recup']=="ok"){
echo '<div id="ok">Vos identifiants ont &eacute;t&eacute; envoy&eacutes sur votre bo&icirc;te email.</div>';
}
if(isset($_GET['session']) && $_GET['session']=="new"){
echo '<div id="ok">Suite à la modification de votre profil, vous devez saisir vos nouvelles donn&eacute;es.</div>';
}
if(isset($_GET['ban']) && $_GET['ban']=="ok"){
echo '<div id="erreur">Votre compte a &eacute;t&eacute; black-list&eacute;!</div>';
}
?>
